            ...
            <form class="form-horizontal tl_admin_add_merchant" method="post" action="#" onsubmit="return addtreatfor();">
              ...
              <div class="box-body">
                    <div class="form-group">
                      <div class="col-sm-4">
                        <label for="inputPassword3" class="tl-about-smallheading">Category</label>
                        <select class="form-control" name="catid" id="catid">
                        <option value="-1">Select Category</option>
                        <?php
                            $relation = DB::table('_relation')->where('parent_cat_id','0')->select('id','name')->where('status','1')->orderBy('id','desc')->get();
                        ?>
                         @foreach($relation AS $relation_val)
                          <option value="{{ $relation_val->id }}"<?php  if(isset($data)&&count($data)>"0") {  if( $catid == $relation_val->id ){ echo 'selected'; }  }?> > {{ $relation_val->name }} </option>  
                          @endforeach
                      </select>
                    </div>
              ...
              <div class="box-body">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" id="treat_for" name="treat_for" onkeypress="return isChar(event)" value="{{$relation_name}}">
              ...
              <i class="fa fa-refresh fa-spin"></i>
                ...
                <a type="button" href="javascript:void(0)" onclick="goBack()" class="tl-btn-defult hvr-sweep-to-right">Go Back</a>
...
function addtreatfor()
{
    ...
    var catid = document.getElementById("catid").value.trim();
     ...
     var _token = $('input[name=_token]').val();
      ...
      if(catid == "-1"){
            document.getElementById('catid').style.border='1px solid #ff0000';
            document.getElementById("catid").focus();
            return false;
        }else{
            document.getElementById('catid').style.border=' ';
        }
